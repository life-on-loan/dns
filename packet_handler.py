...
class DNSPacket:
    def __init__(self, data):
        ...
        if self.answer:
            offset_first, self.rrs_answer = self.get_recs(end_position, 3)
            offset_second, self.rrs_authority = self.get_recs(offset_first, 4)
            self.rrs_additional = self.get_recs(offset_second, 5)[1]
    ...
    def get_recs(self, start_index, index_in_header):
        ...
        for i in range(self.header[index_in_header]):
            ...
            if record_type == 1:  # A
                ...
                adr_ipv6 = struct.unpack(">8H", self.data[offset: offset + 16])
                offset += 16
                str_adr_ipv6 = '%x' % adr_ipv6[0] + ":" + '%x' % adr_ipv6[1] + ":" + '%x' % adr_ipv6[2] + ":" \
                               + '%x' % adr_ipv6[3] + ":" + '%x' % adr_ipv6[4] + ":" + '%x' % adr_ipv6[5]
                list_of_records.append((domain, 12, record_ttl, 16, str_adr_ipv6))
                print("Name: " + str(domain), ", TTL: " + str(record_ttl), ", Address : " + str_adr_ipv6)
            else:
